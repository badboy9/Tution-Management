...
public class PDelete extends JDialog implements ActionListener,FocusListener,ItemListener{
	...
	public JButton bhome,bsrch,bdel;
	...
	public PDelete(JFrame prnt,String title,Boolean bm){
		super(prnt,title,bm);
		...
		ls=new JLabel("Search Payment slip");ls.setFont(new Font("Roman",Font.BOLD,30));
		...
		bhome=new JButton("Home");bhome.addActionListener(this);
		bsrch=new JButton("Search");bsrch.addActionListener(this);
		bdel=new JButton("Delete");bdel.addActionListener(this);	
	...
	public void actionPerformed(ActionEvent e){
		...
		if(b==bsrch){
			...
			if(cb==csid){
				...
				System.out.println("Now Inside Search");
				try{
		...
		if(b==bdel){
			Checkbox cbx=cgres.getSelectedCheckbox();
			int a=Integer.parseInt(cbx.getLabel());
			int sid=Integer.parseInt(lsid[a].getText());
			int pid=Integer.parseInt(lpid[a].getText());
			int amt=Integer.parseInt(lamt[a].getText());
			System.out.println("SID:"+sid+"  PID:"+pid+"  AMT:"+amt);

			try{
				Student sobj=new Student();
				ResultSet rs=srch.sidSearch(sid);
				rs.next();
				sobj.amt=amt+rs.getInt(12);
				sobj.sid=sid;
				updt.payment(sobj);
				del.pDel(pid);
			}
			catch(Exception e1){System.out.println("DELETION$$"+e1);}
